...
def global_thr_test(N, Masked_pixels):
	Q_inj=[]
	ff_array=[]	#pixels fraction that respond to PULSE (fitting purposes)
	ffstd_array=[]	#standard deviation
	thr=[]
	el_noise=[]
	...
	for k in range(160):	#variable carge
		if k == 80:
		...
		pulse_hitmap = np.zeros((512,1024))
		Q_inj.append(((k)*7.06e-3*230e-18)/(1.602e-19)) #(VPULSEH-VPULSEL)*C_inj/e
		...
		for i in range(N):	#N measure
			for j in range(16):	#row region selection
				...
				enablePulserRow_Region(j,0xFFFF,True)	#pulse selected row (16384 pixels)
				maskRow_Region(j,0xFFFF,False)	#unmask selected row (16384 pixels)
				send_broadcast(0x78)  #send PULSE command
				time.sleep(0.0002)
				maskAllPixels(True)	#mask all pixels
				enablePulserAllPixels(False)	#disable Pulser
				enablePulserRow_Region(j+16,0xFFFF,True)	#pulse selected row (16384 pixels)
				maskRow_Region(j+16,0xFFFF,False)	#unmask selected row (16384 pixels)
				send_broadcast(0x78)  #send PULSE command
				time.sleep(0.0002)
				pulse_hitmap = pulse_hitmap + read_PULSE() #cumulative PULSE hitmap
		tt_fraction_fired.append(pulse_hitmap/N)
		pulse_hitmap = []
	...
	for x in range(1024):
		for y in range(512):
			if [y,x] in Masked_pixels:
				...
				for k in range(160):
					if tt_fraction_fired[k][y,x]<=1:
						ff_array.append(tt_fraction_fired[k][y,x])
						ffstd_array.append(math.sqrt(tt_fraction_fired[k][y,x]*(1-tt_fraction_fired[k][y,x])/N))
					else:
						ff_array.append(1)
						ffstd_array.append(0.15)
				fit_error = [max(i,0.0072) for i in ffstd_array]
				if np.sum(ff_array) > 40:	#check if it's a dead pixel
					popt = curve_fit(fit_err_func, Q_inj, ff_array, p0=[200, 10], sigma=fit_error, absolute_sigma=True, bounds=([0, 0.3], [1600, 120]))
					thr.append(popt[0])
					el_noise.append(popt[1])
				else:
...
def read_PULSE():
	...
	send_cmd(read_out)
	while (trailer_detected+empty_detected<2):
		mem_readable = hw.getNode("CSR.status.mem_readable").read()
		hw.dispatch()
		if mem_readable:
...
def decode_block(data_block, n_events, n_idle, n_busy, n_data, region, hitmap_matrix):
	for word in data_block:
		...
		if (word & 0xF00000) == 0xA00000:  # chip header
			...
			ro_flags = (word & 0x0F0000) >> 16
			if ro_flags != 0x0 :
				print ("Read out flags [0x%x] detected on bunch counter  %d\n" % (ro_flags, bc))
		elif (word & 0xF00000) == 0xE00000:  # chip empty
			plane_id = (word & 0x0F0000) >> 16
			bc = (word & 0x00FF00) >> 8
		elif (word & 0xE00000) == 0xC00000:  # reagion header
			...
			n_busy = n_busy + 1
		#else:
	return n_events, n_idle, n_busy, n_data, region, hitmap_matrix
