...
def generate_hitmap_matrix(mem_chunck):
	global n_data 
	global n_events
	global n_idle
	global n_busy
	...
	global plane_id
	global ro_flags
	global ro_flags_list
	global hitmap_matrix
	ro_flags_list = []
	for word in mem_chunck:
		for word in FIFO:
			n_data = n_data + 1
			if (word & 0xF00000) == 0xA00000:  # chip header
				...
				if ro_flags != 0x0 :
					ro_flags_list.append([ro_flags, bc])
...
def pulse_test(mode_cfg):
	...
	for i in range(800):
		...
		if mem_readable:
			...
			hw.dispatch()
			hmap_temp=hitmap_matrix
			generate_hitmap_matrix(FIFO)
	...
	while exit == 0:
		...
		if ip == "pon":
			...
			if initialized:
				...
				for i in range(len(ro_flags_list)):
					file.write("Read out flags [0x%x] detected on bunch counter  %d\n" % (ro_flags_list[i][0], ro_flags_list[i][1]))
