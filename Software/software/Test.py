...
def cc_print():
	print """
		...
		single pixel threshold test	: ttsp
		exit programm			:	exit
	"""
...
def Th_test_sp(x,y):
	...
	enablePulserPixel(x, y, True) #enable pulse 
	region = 0
	n_events = 0
	Trailer_detected = 0
	writeregister(0x0001, 0x208)  # mode cotrol register configuration (see above)
	writeregister(0x0005, 0x0002)  # STROBE duration 3 clk clycles
	writeregister(0x0006, 0x0000)  # gap between 2 STROBES
	writeregister(0x0007, 0x0008)  # delay from PULSE to STROBE 8+1 clk cycles
	writeregister(0x0008, 0x000E)  # PULSE duration 12 clk cylces
	...
	while exit == 0:
		...
		if ip == "pon":
			...
			if initialized:
				writeregister(0x0004, 0x0060)  # Analog pulse, automatic strobe after pulse command
				writeregister(0x0606, 0x0000)  # Set PULSEL to 0.37V (DAC offset)
				ydata_pulse = []  # initialize empty array
				xdata_pulse = []  # initialize empty array
				yerror_pulse = []  # initialize empty array
				for k in range (120):
					Q_inj = ((k)*7.06e-3*230e-18)/(1.602e-19)
					writeregister(0x0605,k)  # Set PULSEH to k*7.06 mV
					nevents = Th_test_sp(35,460)
					ydata_pulse.append(nevents)
					if nevents != 0:
						yerror_pulse.append(1/nevents*math.sqrt((nevents*500-nevents*nevents)/(500)))
					else:
						yerror_pulse.append(0.00001)
					xdata_pulse.append(Q_inj)
				fit_error = [max(i,0.0008) for i in yerror_pulse]
				popt, pcov = curve_fit(fit_func, xdata_pulse, ydata_pulse, p0=[650, 38], sigma=fit_error, absolute_sigma=True, bounds=([400,30], [1200,80]))
				print 'Threshold= %f+/-%f' % (popt[0], math.sqrt(pcov[0,0]))
				print 'El_noise= %f+/-%f' % (popt[1], math.sqrt(pcov[1,1]))
